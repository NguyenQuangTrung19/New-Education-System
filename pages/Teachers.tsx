...
export const Teachers: React.FC<TeachersProps> = ({ currentUser }) => {
  ...
  const [formTeacher, setFormTeacher] =
    useState<Partial<Teacher>>(defaultTeacherState);

  const isAdmin = currentUser?.role === UserRole.ADMIN;

  const calculateExperience = (joinYear: number) =>
    Math.max(0, new Date().getFullYear() - joinYear);
  const availableSubjects = Array.from(
    new Set(MOCK_SUBJECTS.map((s) => s.name))
  );
  ...
  const handleOpenAdd = () => {
    ...
    setFormTeacher({
      ...defaultTeacherState,
      id: `GV${String(Math.floor(Math.random() * 10000)).padStart(4, "0")}`,
    });
    setFormErrors([]);
    setIsFormOpen(true);
  };
  ...
  const validateTeacherForm = (data: Partial<Teacher>): string[] => {
    ...
    if (!editingTeacher && !data.password?.trim())
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    if (editingTeacher) {
      setTeachers(
        teachers.map((t) => (t.id === editingTeacher.id ? teacherData : t))
      );
    } else {
  ...
  const handleAddSubjectToForm = (subject: string) => {
    if (subject && !formTeacher.subjects?.includes(subject)) {
      setFormTeacher({
        ...formTeacher,
        subjects: [...(formTeacher.subjects || []), subject],
      });
    }
  };
  ...
  const handleAddNote = (teacherId: string, note: string) => {
    ...
    const updatedTeachers = teachers.map((t) =>
      t.id === teacherId ? { ...t, notes: [...(t.notes || []), note] } : t
    );
  ...
  const TeacherDetailModal = ({
    ...
    const assignedClasses = MOCK_CLASSES.filter(
                  ...
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div>
                      ...
                      <div className="flex flex-wrap items-center gap-3 mt-2 text-sm">
                        <span className="px-2.5 py-0.5 bg-indigo-100 text-indigo-700 rounded-full font-medium border border-indigo-200">
                          {teacher.id}
                        </span>
                        <span className="flex items-center text-gray-500">
                          <Briefcase className="h-4 w-4 mr-1" />{" "}
                          {t("teachers.modal.joined")} {teacher.joinYear}
                        </span>
                        <span className="flex items-center text-gray-500">
                          <Clock className="h-4 w-4 mr-1" /> {experience}{" "}
                          {t("teachers.modal.exp")}
                        </span>
                      </div>
                    </div>
                    {isAdmin && (
                      <div className="flex gap-2">
                        <button
                          ...
                          className="px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition shadow-sm font-medium flex items-center text-sm"
                        >
                  ...
                  <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
                    <div className="px-6 py-4 border-b border-gray-100 bg-gray-50 flex items-center gap-2">
                      ...
                      <h3 className="text-sm font-bold text-gray-800 uppercase">
                        {t("teachers.modal.personal")}
                      </h3>
                    </div>
                    <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-y-6 gap-x-8">
                      <div>
                        <label className="text-xs text-gray-400 font-medium uppercase block mb-1">
                          {t("teacher.email")}
                        </label>
                        <div className="text-sm font-medium text-gray-900 flex items-center break-all">
                      ...
                      <div>
                        ...
                        <div className="text-sm font-medium text-gray-900 flex items-center">
                          <Calendar className="h-4 w-4 mr-2 text-indigo-400 shrink-0" />{" "}
                          {teacher.dateOfBirth} (
                          {calculateAge(teacher.dateOfBirth)} years)
                        </div>
                      </div>
                      <div>
                        <label className="text-xs text-gray-400 font-medium uppercase block mb-1">
                        ...
                        <div className="text-sm font-medium text-gray-900 flex items-start">
                          <MapPin className="h-4 w-4 mr-2 text-indigo-400 mt-0.5 shrink-0" />{" "}
                          {teacher.address}
                        </div>
                      </div>
                      <div>
                        ...
                        <div className="text-sm font-medium text-gray-900 flex items-center">
                          <CreditCard className="h-4 w-4 mr-2 text-indigo-400 shrink-0" />{" "}
                          {teacher.citizenId}
                        </div>
                      </div>
                      <div>
                ...
                <div className="space-y-8">
                      ...
                      <div className="p-6">
                        <div className="flex items-center justify-between mb-4">
                          <div>
                            ...
                            <p className="font-mono font-bold text-gray-800 bg-gray-100 px-2 py-1 rounded text-sm">
                              {teacher.username}
                            </p>
                          </div>
                  ...
                  <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
                    <div className="px-6 py-4 border-b border-gray-100 bg-gray-50">
                      <h3 className="text-sm font-bold text-gray-800 uppercase">
                        {t("teachers.modal.teachingSubjects")}
                      </h3>
                    </div>
                    <div className="p-6">
                    ...
                    <div className="p-6">
                      <div className="space-y-3 mb-4 max-h-48 overflow-y-auto pr-1 custom-scrollbar">
                        {teacher.notes?.map((note, idx) => (
                          <div
                            key={idx}
                            className="p-3 bg-white rounded-lg text-sm text-gray-600 shadow-sm border border-gray-100"
                          >
                            {note}
                          </div>
                        ))}
                      ...
                      {isAdmin && (
                        <div className="flex gap-2">
                          <input
                            ...
                            onKeyDown={(e) =>
                              e.key === "Enter" &&
                              (handleAddNote(teacher.id, noteInput),
                              setNoteInput(""))
                            }
                          ...
                          <button
                            ...
                            className="p-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 shadow-sm shrink-0 flex items-center justify-center"
                          >
            ...
            <div className="h-full flex flex-col bg-white shadow-2xl animate-slide-in-right">
                ...
                <div className="flex items-start justify-between">
                  <div>
                    <h2 className="text-xl font-bold">
                      {editingTeacher
                        ? t("teachers.form.edit")
                        : t("teachers.form.new")}
                    </h2>
                    <p className="text-indigo-100 text-sm mt-1">
                      {editingTeacher
                        ? `Updating ${editingTeacher.id}`
                  ...
                  {formErrors.length > 0 && (
                    <div className="bg-red-50 border border-red-200 rounded-xl p-4 animate-shake">
                      <h4 className="flex items-center text-red-700 font-bold text-sm mb-2">
                        <AlertTriangle className="h-4 w-4 mr-2" /> Vui lòng kiểm
                        tra lại:
                      </h4>
                    ...
                    <div className="space-y-4">
                      <div>
                        ...
                        <input
                          type="text"
                          required
                          className="w-full px-4 py-2.5 bg-gray-50 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:bg-white transition-all text-sm outline-none"
                      ...
                      <div>
                        ...
                        <input
                          type="password"
                          placeholder={editingTeacher ? "Unchanged" : "••••••"}
                          className="w-full px-4 py-2.5 bg-gray-50 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:bg-white transition-all text-sm outline-none"
                  ...
                  <div className="bg-white p-5 rounded-xl border border-gray-200 shadow-sm space-y-4">
                    ...
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) => {
                          handleAddSubjectToForm(e.target.value);
                          e.target.value = "";
                        }}
                      >
                      ...
                      <div className="flex flex-wrap gap-2">
                        {formTeacher.subjects?.map((sub) => (
                          <span
                            key={sub}
                            className="inline-flex items-center px-3 py-1 rounded-full text-xs font-bold bg-indigo-100 text-indigo-700"
                          >
                            {sub}
                            <button
                              type="button"
                              onClick={() =>
                                setFormTeacher({
                                  ...formTeacher,
                                  subjects: formTeacher.subjects?.filter(
                                    (s) => s !== sub
                                  ),
                                })
                              }
              ...
              <div className="px-6 py-4 bg-gray-50 border-t border-gray-200 shrink-0 flex justify-end gap-3">
                <button
                  type="button"
                  onClick={() => setIsFormOpen(false)}
                  className="px-5 py-2.5 text-gray-700 font-medium hover:bg-gray-200 rounded-lg transition-colors text-sm"
                >
                  {t("common.cancel")}
                </button>
                <button
                  onClick={handleSave}
                  type="button"
                  className="px-5 py-2.5 bg-indigo-600 text-white font-medium hover:bg-indigo-700 rounded-lg shadow-lg shadow-indigo-500/30 transition-all transform active:scale-95 text-sm flex items-center"
                >
        ...
        <div className="flex gap-3 w-full md:w-auto">
          ...
          {isAdmin && (
            <button
              ...
              className="bg-indigo-600 hover:bg-indigo-700 text-white px-5 py-2.5 rounded-xl font-medium shadow-lg shadow-indigo-500/30 transition-all flex items-center whitespace-nowrap"
            >
                  ...
                  <td className="px-6 py-4">
                    <div className="flex flex-wrap gap-1.5 max-w-[200px]">
                      {teacher.subjects.slice(0, 3).map((subj, idx) => (
                        <span
                          key={idx}
                          className="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold uppercase bg-gray-100 text-gray-600 border border-gray-200"
                        >
                          {subj}
                        </span>
                      ))}
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md w-screen h-screen">
            <div className="bg-white rounded-xl shadow-xl w-full max-w-sm p-6 animate-scale-in">
              ...
              <p className="text-sm text-gray-500 mb-4">
                {t("security.enterPass")}{" "}
                <strong>{teacherToReveal?.name}</strong>.
              </p>
              ...
              {!revealedPassword ? (
                ...
                <div className="space-y-4">
                  ...
                  <button
                    onClick={() => setSecurityModalOpen(false)}
                    className="w-full py-2 text-gray-600 bg-gray-100 rounded-lg hover:bg-gray-200"
                  >
                    Close
                  </button>
                </div>
