  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      if (role === UserRole.ADMIN) {
        ...
        const teacher =
          MOCK_TEACHERS.find((t) => t.email === email) || MOCK_TEACHERS[0];
        user = {
          id: teacher.id,
          name: teacher.name,
          email: teacher.email,
        ...
        const student =
          MOCK_STUDENTS.find((s) => s.email === email) || MOCK_STUDENTS[0];
        user = {
          id: student.id,
          name: student.name,
          email: student.email,
            ...
            <div className="absolute -bottom-10 left-8 flex items-end">
              <div className="w-20 h-20 bg-white rounded-2xl p-2 shadow-lg">
                <img
                  ...
                  onError={(e) => {
                    e.currentTarget.src = SCHOOL_INFO.logoFallback;
                  }}
                  alt="Logo"
              ...
              <div className="space-y-6 animate-fade-in">
                <div className="prose prose-sm max-w-none text-gray-600">
                  ...
                  <h3 className="flex items-center text-lg font-bold text-gray-900 mb-3">
                    ...
                    {t("guest.about.achievements")}
                  </h3>
                  <ul className="space-y-2 list-none pl-0">
                    <li className="flex items-start bg-white p-3 rounded-xl border border-gray-100 shadow-sm">
                      <div className="h-5 w-5 rounded-full bg-yellow-100 text-yellow-600 flex items-center justify-center mr-3 shrink-0 text-xs font-bold">
                        1
                      </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-shadow">
                    ...
                    <h4 className="font-bold text-gray-900 text-sm uppercase mb-1">
                      {t("guest.contact.address")}
                    </h4>
                    <p className="text-sm text-gray-600">
                      123 Đường Giáo Dục, Phường Phước Tân, TP. Biên Hòa, Đồng
                      Nai
                    </p>
                  </div>
                  ...
                  <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-shadow">
                    ...
                    <p className="text-sm text-gray-600">
                      contact@thcsphuoctan.edu.vn
                    </p>
                    <p className="text-sm text-gray-600">
                      admissions@thcsphuoctan.edu.vn
                    </p>
                  </div>
                  ...
                  <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-shadow">
                    ...
                    <h4 className="font-bold text-gray-900 text-sm uppercase mb-1">
                      {t("guest.contact.social")}
                    </h4>
                    <p className="text-sm text-indigo-600 hover:underline cursor-pointer">
                      www.thcsphuoctan.edu.vn
                    </p>
                    <div className="flex gap-3 mt-2">
                      <Facebook className="h-5 w-5 text-blue-700 cursor-pointer hover:scale-110 transition-transform" />
                      <div className="h-5 w-5 bg-red-600 rounded text-white flex items-center justify-center font-bold text-[10px] cursor-pointer hover:scale-110 transition-transform">
                        YT
                      </div>
                    </div>
                  </div>
            ...
            {activeInfoTab === "credit" && (
              <div className="space-y-6 animate-fade-in flex flex-col items-center justify-center py-4">
                ...
                <div className="text-center space-y-2">
                  ...
                  <p className="text-indigo-600 font-bold bg-indigo-50 px-3 py-1 rounded-full text-xs inline-block uppercase tracking-wider">
                    {t("guest.credit.role")}
                  </p>
                ...
                <div className="flex gap-4 pt-4">
                  ...
                  <a
                    href="#"
                    className="p-3 bg-blue-50 rounded-full text-blue-600 hover:bg-blue-100 hover:text-blue-800 transition-colors"
                  >
                    <Linkedin className="h-5 w-5" />
                  </a>
                  <a
                    href="#"
                    className="p-3 bg-indigo-50 rounded-full text-indigo-600 hover:bg-indigo-100 hover:text-indigo-800 transition-colors"
                  >
                ...
                <div className="mt-8 text-xs text-gray-400 flex items-center">
                  Made with{" "}
                  <Heart className="h-3 w-3 text-red-500 mx-1 fill-current animate-pulse" />{" "}
                  for Education
                </div>
              </div>
            )}
      ...
      <div className="relative z-10 w-full max-w-md">
        <div className="bg-white/10 backdrop-blur-xl p-8 rounded-[2rem] shadow-2xl border border-white/40 animate-fade-in-up relative overflow-hidden">
          ...
          <div className="text-center mb-8 relative z-10">
            ...
            <h1 className="text-2xl md:text-3xl font-black text-white tracking-tight uppercase drop-shadow-md">
              {SCHOOL_INFO.name}
            </h1>
          ...
          <div className="bg-black/20 p-1.5 rounded-2xl mb-8 flex relative backdrop-blur-sm border border-white/10">
            {[
              ...
              },
              {
                r: UserRole.TEACHER,
                icon: User,
                label: t("login.role.teacher"),
              },
              {
                r: UserRole.STUDENT,
                icon: GraduationCap,
                label: t("login.role.student"),
              },
            ].map((item) => (
              <button
                key={item.r}
                ...
                className={`flex-1 flex items-center justify-center py-2.5 text-xs font-bold rounded-xl transition-all duration-300 relative z-10 ${
                  role === item.r
                    ? "bg-ice-white text-glacier-dark shadow-lg scale-105"
                    : "text-white/70 hover:text-white hover:bg-white/5"
                }`}
              >
                <item.icon
                  className={`h-4 w-4 mr-1.5 ${
                    role === item.r ? "text-glacier-lake" : ""
                  }`}
                />
                {item.label}
              </button>
        ...
        <div className="text-center mt-8">
          <p className="text-white/50 text-xs font-medium">
            © 2024 {SCHOOL_INFO.name}. {t("login.developer")}{" "}
            <span className="text-white/80">Nguyễn Quang Trung</span>
          </p>
        </div>
      </div>
