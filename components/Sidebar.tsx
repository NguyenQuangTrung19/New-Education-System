...
export const Sidebar: React.FC<SidebarProps> = ({ 
  ...
  return (
    <aside 
      className={`fixed top-0 left-0 z-40 h-screen transition-all duration-500 ease-in-out flex flex-col border-r border-white/20 shadow-[4px_0_24px_rgba(128,177,211,0.25)] ${
        isOpen ? 'translate-x-0' : '-translate-x-full'
      } md:translate-x-0 ${isCollapsed ? 'w-[88px]' : 'w-[280px]'}`}
      style={{
        // Glacier Lake Gradient: PANTONE 16-4118 TCX base
        background: 'linear-gradient(170deg, #80B1D3 0%, #5D8BAE 100%)' 
      }}
    >
      {/* Header Logo Area */}
      <div className={`h-24 flex items-center relative ${isCollapsed ? 'justify-center px-0' : 'px-8'}`}>
        ...
        {!isCollapsed && (
          <div className="ml-4 flex flex-col justify-center animate-fade-in-up">
            <span className="font-extrabold text-xl tracking-tight text-white leading-none font-sans drop-shadow-sm">{SCHOOL_INFO.name}</span>
          ...
          return (
            <button
              ...
              className={`w-full flex items-center py-3.5 rounded-2xl transition-all duration-300 group relative ${
                ...
                isActive 
                  ? 'bg-white text-glacier-dark shadow-[0_4px_15px_rgba(0,0,0,0.1)] font-bold' 
            ...
            >
              ...
              <Icon 
                className={`h-[22px] w-[22px] transition-all duration-300 ${
                    isActive ? 'text-glacier-dark scale-110' : 'group-hover:scale-110'
                } ${!isCollapsed ? 'mr-3.5' : ''}`} 
                strokeWidth={isActive ? 2.5 : 2}
              />
              
              {!isCollapsed && (
                  <span className={`text-[14px] tracking-wide ${isActive ? '' : ''}`}>
                      {item.label}
                  </span>
              )}
              ...
              {isCollapsed && (
                <div className="absolute left-full ml-4 px-3 py-2 bg-slate-800 text-white text-xs font-bold rounded-lg opacity-0 group-hover:opacity-100 pointer-events-none transition-all duration-200 z-50 whitespace-nowrap shadow-xl translate-x-2 group-hover:translate-x-0">
                  {item.label}
      ...
      {/* Footer / Toggle */}
      <div className="p-6 border-t border-white/20 bg-black/5 backdrop-blur-sm">
         {/* Logout */}
        <button 
          onClick={onLogout}
        ...
        >
          <LogOut className={`h-5 w-5 transition-transform group-hover:-translate-x-1 ${!isCollapsed ? 'mr-3' : ''}`} />
        ...
        <button 
           ...
           className="hidden md:flex w-full mt-4 items-center justify-center py-2 text-white/50 hover:text-white hover:bg-white/10 rounded-lg transition-colors"
        >
